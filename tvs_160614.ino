...
void process_data(const char* data) {
  ...
  buf[sizeof(buf)] += vitesse; // On remplit le buffer
  Serial.print("BUFFER : ");
  for (int o = 0; o < sizeof(buf); o++) {
    Serial.println(buf[o]);
  }
}
...
void loop() {
  switch(protocole) {
  case 0: // Rien pour l'instant, tests et calibrages plus tard
    protocole++;
  case 1: // GENERAL
    if (Serial.available() > 0) {
      processIncomingByte(Serial.read());
      if (buf[sizeof(buf)]!=0) changeVitesse(buf[sizeof(buf)]);
      respire();
      buf[sizeof(buf)] = 0; // On vide le buffer a chaque cycle
    }
  default:
    break;
  }
