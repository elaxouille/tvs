...
try:
	while 1:
		time.sleep(1) # Pour ne pas surcharger l'arduino
		depth = freenect.sync_get_depth()[0]
		...
		if (isConnected):
			print ser.readline()
			if (result > 99) :
				print "\033[0;31m[PYT]\033[0m\tANGOISSE\tENVOI : 99"
				ser.write("99\n")
				print "\033[0;31m[PYT]\033[0m\t",str(result).format('02d')
			elif (result < 6) :
				print "\033[0;31m[PYT]\033[0m\tCALME\tENVOI : 06"
				ser.write("06\n")
				print "\033[0;31m[PYT]\033[0m\t",str(result).format('02d')
			else :
				...
				ser.write(envoi)
				ser.write('\n')
				print "\033[0;31m[PYT]\033[0m\tENVOI",envoi
		else :
